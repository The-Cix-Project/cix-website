...
The reproduced workflow remains captured from Cix commit `4d5f4ffa167520f0e4a64825b4c6c9a5a28942c7` (2026-09-01 20:07:21 +01:00). The current product contract reviewed for this website update is Cix commit `043eb07f` (2026-09-25), including the unified package/image/deployment/host pipeline, configuration documents with section-level diff/apply, the single management-address model, Cix’s own EFI boot manager, online data-directory partition growth, measured container disk usage, the shell/PBS recipe split with CPDL-backed builds, the separate flat `cix-recipes` corpus, credential-regression protection, safe archive extraction, and three Cix-host floor tests gating a release. The source repositories also publish the Apache-2.0 licence, DCO contribution guide, and Cix trademark policy.
