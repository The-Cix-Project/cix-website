...
- Refreshed the product alignment against Cix `a06145ed` / live `v2.57.238`: documented the separate flat `cix-recipes` corpus, refreshed PBS/CPDL and CBS execution claims, recorded credential-regression protection, safe archive extraction, compiler-runtime preservation, and Cix-host floor tests gating releases, corrected the build-system boundary, refreshed status provenance, and updated the homepage host identity.
